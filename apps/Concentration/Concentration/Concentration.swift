...
struct Concentration
{
    ...
    mutating func chooseCard(at index: Int) {
        assert(cards.indices.contains(index), "Concentration.chooseCard(\(index)): Chosen index not in range")
    ...
    init(numCardPairs: Int) {
        assert(numCardPairs > 0, "Concentration.init(\(numCardPairs)): Must have one pair of cards")
